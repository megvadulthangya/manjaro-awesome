...
# CRITICAL: Add scripts directory to sys.path for module imports
SCRIPT_DIR = Path(__file__).parent.absolute()
if str(SCRIPT_DIR) not in sys.path:
    sys.path.insert(0, str(SCRIPT_DIR))

print(">>> DEBUG: Script started, SCRIPT_DIR:", SCRIPT_DIR)
...
# Import our modules from the modules package
try:
    ...
    logger.info("✅ All modules imported successfully from modules package")
...
# Try to import our config files from the same directory
try:
    import config
    import packages
    HAS_CONFIG_FILES = True
    print(f"✅ Config files loaded from {SCRIPT_DIR}/")
except ImportError as e:
    ...
    def _init_modules(self):
        ...
        try:
            ...
            repo_config = {
                ...
                'vps_user': self.vps_user,
                'vps_host': self.vps_host,
                'ssh_repo_url': getattr(config, 'SSH_REPO_URL', 'git@github.com:megvadulthangya/manjaro-awesome.git') if HAS_CONFIG_FILES else 'git@github.com:megvadulthangya/manjaro-awesome.git',
            }
