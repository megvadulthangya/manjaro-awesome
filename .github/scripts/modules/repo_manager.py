...
# Try to import config from the scripts directory
try:
    import config
    HAS_CONFIG = True
except ImportError:
...
class RepoManager:
    ...
    def __init__(self, config_dict: Dict):
        ...
        Args:
            config_dict: Dictionary containing:
                ...
                - ssh_repo_url: SSH repository URL for cloning
        """
        self.repo_name = config_dict['repo_name']
        self.output_dir = Path(config_dict['output_dir'])
        self.remote_dir = config_dict['remote_dir']
        self.mirror_temp_dir = Path(config_dict.get('mirror_temp_dir', '/tmp/repo_mirror'))
        self.vps_user = config_dict['vps_user']
        self.vps_host = config_dict['vps_host']
        self.ssh_repo_url = config_dict.get('ssh_repo_url', 'git@github.com:megvadulthangya/manjaro-awesome.git')
        
        # Extract repo name for dynamic workspace resolution
        self.extracted_repo_name = self._extract_repo_name(self.ssh_repo_url)
        
        # STRICT PATH COMPLIANCE: Dynamic workspace resolution
        self.base_temp_dir = Path(f"/tmp/{self.extracted_repo_name}_build_temp")
        self.state_tracking_dir = self.base_temp_dir / ".build_tracking"
        self.git_workflow_dir = self.base_temp_dir / "git_workflow"
        
        # Ensure directories exist on instantiation (STRICT REQUIREMENT)
        self.base_temp_dir.mkdir(parents=True, exist_ok=True)
        self.state_tracking_dir.mkdir(parents=True, exist_ok=True)
        self.git_workflow_dir.mkdir(parents=True, exist_ok=True)
        
        logger.info(f"📁 STRICT PATH COMPLIANCE:")
        logger.info(f"  Base temp: {self.base_temp_dir}")
        logger.info(f"  State tracking: {self.state_tracking_dir}")
        logger.info(f"  Git workflow: {self.git_workflow_dir}")
    ...
    def _extract_repo_name(self, ssh_repo_url: str) -> str:
        """Extract repository name from SSH URL for dynamic workspace resolution"""
        ...
        logger.info(f"Repository name extracted from URL: {repo_name}")
        return repo_name
