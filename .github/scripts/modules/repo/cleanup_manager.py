...
- Orphaned signature cleanup (local + VPS)
"""
...
class CleanupManager:
    ...
    3. Orphaned signature cleanup (local + VPS)
    """
    ...
    def get_vps_files_to_delete(self, version_tracker) -> Tuple[List[str], List[str]]:
        ...
        NOTE: version_tracker is currently not used here; kept for compatibility.

        Returns:
            Tuple of (files_to_delete, files_to_keep)
        """
        _ = version_tracker  # compatibility placeholder
    ...
    def _remove_orphaned_signatures(self):
        """Remove orphaned .sig files that don't have a corresponding package (local output_dir)."""
    ...
    def cleanup_vps_orphaned_signatures(self) -> Tuple[int, int, int]:
        ...
        package_files, signature_files, orphaned_signatures = self._analyze_vps_files_for_orphans(vps_files)
        ...
        deleted_count = self._delete_remote_in_batches(orphaned_signatures)
        ...
        logger.info(f"  deletion_exit_status: {0 if deleted_count == len(orphaned_signatures) else 1}")

        if deleted_count == len(orphaned_signatures):
    ...
    def server_cleanup(self, version_tracker):
        ...
        logger.info(
            f"Zero-Residue cleanup initiated with {len(version_tracker._package_target_versions)} target versions"
        )
        ...
        for vps_file in vps_files:
            ...
            is_db_or_sig = any(
                filename.endswith(ext)
                for ext in [".db", ".db.tar.gz", ".sig", ".files", ".files.tar.gz"]
            )
            if is_db_or_sig:
            ...
            if pkg_name in version_tracker._package_target_versions:
                ...
                if pkg_name in getattr(version_tracker, "_skipped_packages", set()):
                    files_to_keep.add(filename)
        ...
        deleted_count = self._delete_remote_in_batches(files_to_delete)
    ...
    def _cleanup_orphaned_signatures_vps(self, vps_files: Optional[List[str]] = None) -> int:
        ...
        for sig_path in orphaned_signatures:
            logger.info(f"Orphaned signature: {Path(sig_path).name} (package missing)")
        ...
        if deleted_count == len(orphaned_signatures):
            ...
            logger.error(f"❌ Deleted {deleted_count}/{len(orphaned_signatures)} orphaned signatures from VPS")

        return deleted_count

    def _analyze_vps_files_for_orphans(
        self, vps_files: List[str]
    ) -> Tuple[Set[str], List[str], List[str]]:
        """
        Analyze VPS file list and determine orphaned signature files.

        Returns:
            (package_files_set, signature_files_list, orphaned_signature_paths)
        """
        ...
        orphaned_signatures: List[str] = []
        for sig_path in signature_files:
            sig_filename = Path(sig_path).name
            pkg_filename = sig_filename[:-4]  # remove ".sig"

            # Only treat signatures for package archives as candidates (ignore db sigs)
            if pkg_filename.endswith((".pkg.tar.zst", ".pkg.tar.xz")):
                if pkg_filename not in package_files:
                    orphaned_signatures.append(sig_path)

        return package_files, signature_files, orphaned_signatures

    def _delete_remote_in_batches(self, files_to_delete: List[str], batch_size: int = 50) -> int:
        """
        Delete remote files in batches.

        Returns:
            Number of files successfully deleted (best-effort).
        """
        if not files_to_delete:
            return 0
    ...
    def _get_vps_file_inventory(self) -> Optional[List[str]]:
        ...
        # Quote remote_dir safely for remote shell
        remote_dir_q = shlex.quote(self.remote_dir)

        # One-liner command for predictable shell behavior
        remote_cmd = (
            ...
            r'-o -name "*.abs.tar.gz" \) 2>/dev/null'
        )
    ...
    def _delete_files_remote(self, files_to_delete: List[str]) -> bool:
        """Delete files from remote server (hardened quoting)."""
        if not files_to_delete:
            return True

        # Shell-quote each remote path safely; add -- to prevent option injection
        quoted_files = " ".join(shlex.quote(f) for f in files_to_delete)
        delete_cmd = f"rm -fv -- {quoted_files}"
